 ...
 * @version 1.0
 */
// Ngo Duc Trung - Da lam phan: 1+2+3+4+7
...
void skipComment() {
  do{
    readChar();
  }while(currentChar!= -1 && charCodes[currentChar]!= CHAR_TIMES);
  if(currentChar ==-1){
...
Token* readIdentKeyword(void) {
  ...
  token->tokenType = checkKeyword(token->string);//kiem tra co phai token
  if (token->tokenType == TK_NONE)//kiem tra loi
