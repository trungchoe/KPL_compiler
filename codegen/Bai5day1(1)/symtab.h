...
struct VariableAttributes_ {
  ...
  int localOffset;        // vi tri tinh tu base cua frame
};
...
struct ProcedureAttributes_ {
  ...
  int paramCount; //so luong tham so
  CodeAddress codeAddress;  //vi tri bat dau cua cai khai bao do
};
...
struct ProgramAttributes_ {
  struct Scope_ *scope;
  CodeAddress codeAddress; //vi tri bat dau cua chuong trinh chinh
};
...
struct Scope_ {
  ...
  Object *owner;
  struct Scope_ *outer;
  int frameSize; //kich thuoc frame
};
